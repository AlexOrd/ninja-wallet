...
exports.validatorFactory = function (schemaName) {
  return function (...data) {
    ...
    const { error } = validationSchemas.auth[schemaName].validate(...data);
  ...
  function detectErrorType(error) {
    ...
    if (getErrType(error) === 'any.required' || getErrType(error) === 'object.and') {
      ...
      return { err: new ResponseError('LACK_NEEDED_FIELD', 401, errMessage) };
    }

    if (getErrType(error) === 'object.unknown') {
      const errMessage = `passed object CONTAIN excess field(s), ${getErrMessage(error)}`;
      return { err: new ResponseError('EXCESS_FIELD', 401, errMessage) };
    }

    const errMessage = `passed data is invalid, ${getErrMessage(error)}`
    return { err: new ResponseError('INVALID_DATA', 401, errMessage)};
  }
};
